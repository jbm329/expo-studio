...
def test_null_file_service(null_file_service, parent_widget):
    ...
    path, filter = null_file_service.get_open_filename(
        parent_widget, OpenFileRequest(title="Open", initial_path="", filter_str="All (*.*)")
    )
    ...
    path, filter = null_file_service.get_save_filename(
        parent_widget, SaveFileRequest(title="Save", initial_path="", filter_str="All (*.*)")
    )
    assert path == "save.txt"

    # Test directory
    null_file_service.enqueue_directory_response("/tmp")
    path = null_file_service.get_existing_directory(parent_widget, DirectoryRequest(title="Dir", initial_path=""))
    assert path == "/tmp"
...
def test_qt_file_service_open(mock_fd_class, qt_file_service, parent_widget):
    ...
    mock_fd_class.getOpenFileName.assert_called_once_with(parent_widget, "Select File", "", "All (*.*)")
...
def test_qt_file_service_save(mock_fd_class, qt_file_service, parent_widget):
    ...
    mock_fd_class.getSaveFileName.assert_called_once_with(parent_widget, "Save File", "default.txt", "All (*.*)")
...
def test_qt_file_service_directory(mock_fd_class, qt_file_service, parent_widget):
    ...
    mock_fd_class.getExistingDirectory.assert_called_once_with(parent_widget, "Choose Dir", "")
