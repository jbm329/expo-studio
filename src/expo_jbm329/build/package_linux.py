...
def _create_checksum(artifact_path: Path) -> bool:
    ...
    try:
        ...
        print(f"[package-linux] Failed to create checksum: {exc}", file=sys.stderr)
        return False

    if not checksum_path.is_file():
        print(f"[package-linux] Checksum file was not created: {checksum_path}", file=sys.stderr)
        return False

    print(f"[package-linux] SHA256: {checksum_path}")
    return True
